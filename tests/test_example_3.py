...
def test_example_3_power_func():
    ...
    assert np.sqrt(b_cov[0, 0]) == approx(1.7821e-2, abs=0.0432e-2) # 1.8252e-2
    # b1
    assert b[1] == approx(5.1213e-4, abs=0.0003e-4)
    assert np.sqrt(b_cov[1, 1]) == approx(2.3693e-5, abs=0.0657e-5) # 2.4349e-5
    # b2
    assert b[2] == approx(8.4986e-2, abs=0.0005e-2)
    assert np.sqrt(b_cov[2, 2]) == approx(4.9745e-3, abs=0.1428e-3) # 5.1173e-3
    # covariance between coefficients
    assert b_cov[0, 1] == approx(-3.8265e-7, abs=0.2213e-7) # -4.0477e-7
    assert b_cov[0, 2] == approx(7.9326e-5, abs=0.4641e-5) # 8.3966e-5
    assert b_cov[1, 2] == approx(-1.1780e-7, abs=0.0662e-7) # -1.2441e-7
    # residual
    ...
    assert np.sqrt(x_cov[0, 0]) == approx(1.4141e-2, abs=0.0041e-2) # 1.4181e-2
