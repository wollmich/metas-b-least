...
def test_example_2_linear_func():
    ...
    assert np.sqrt(b_cov[1, 1]) == approx(2.4161e-8, abs=0.0002e-8) # 2.4162e-3
    # covariance between intercept and slope
    assert b_cov[0, 1] == approx(-7.2747e-12, abs=0.0098e-12) # -7.2844e-12
    # residual
    ...
    assert np.sqrt(x_cov[0, 0]) == approx(2.0244e-3, abs=0.0002e-3) # 2.0242e-3
    # mixture no 2
    assert x[1] == approx(8.9863, abs=0.0005) # 8.9859
    assert np.sqrt(x_cov[1, 1]) == approx(9.9718e-3, abs=0.0001e-3) # 9.9717e-3
...
def test_example_2_second_order_poly():
    ...
    assert b_cov[0, 2] == approx(4.667e-17, abs=0.003e-17)
    ...
    assert np.sqrt(x_cov[0, 0]) == approx(3.2910e-3, abs=0.0006e-3) # 3.2905e-3
    # mixture no 2
    assert x[1] == approx(8.9727, abs=0.0004) # 8.9723
    assert np.sqrt(x_cov[1, 1]) == approx(1.1762e-2, abs=0.0001e-2)
